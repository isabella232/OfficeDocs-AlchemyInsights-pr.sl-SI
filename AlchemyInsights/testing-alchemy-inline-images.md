---
title: Preskušanje alkimije inline slike
ms.author: pebaum
author: pebaum
ms.date: 11/16/2017
ms.audience: ITPro
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.assetid: 28d8036f-9df2-4dff-835b-9424bfe988a2
ms.custom:
- "273"
- "1500006"
ms.openlocfilehash: 8a9d98e2af3ef0cc71b820b4e74aa2a2a522346b
ms.sourcegitcommit: 0b06093dabd685f76cc39b1d7c0f8b03883b6e79
ms.translationtype: MT
ms.contentlocale: sl-SI
ms.lasthandoff: 10/25/2019
ms.locfileid: "36743145"
---
# <a name="access-the-domains-page"></a>Dostop do strani domen

Če želite dostopati do strani domen, preprosto kliknite to ikono spodaj:

  
<img src='data:image/png;base64,'/>

Nato lahko dodate, spreminjate ali odstranite domeno.
Če želite več informacij, glejte [pogosta vprašanja o domenah.](https://docs.microsoft.com/office365/admin/setup/domains-faq)
  